...
void ButtonPressed() {
  if (digitalRead(KNOPF_LESER) == HIGH) {
    ...
    tft.fillScreen(ST7735_BLACK);
    if (zustand == 1) {
      ScreenText(" Version 1                                  Speaker");
    }
    if (zustand == 2) {
      ScreenText(" Version 2                                  Dreiecke");
    }
    if (zustand == 3) {
      ScreenText(" Version 3                                 Spektrum");
    }
    if (zustand == 4) {
      ScreenText("  Version 4                              Drei bewegende           Kreise");
    }
    if (zustand == 5) {
      ScreenText("  Version 5                              Ein bewegender           Kreis");
    }
...
void Dreiecke() {
  ...
  //tft.drawTriangle(w, y, y, x, z, x, ConvertRGB(Red, Green, 0));
